...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        avg_cost = 0.
        sess.run(optimizer, feed_dict = {X: train_x, Y: train_y, dropout_rate: 0.5})
        # Compute average loss
        avg_cost = sess.run(cost, feed_dict={X: train_x, Y: train_y, dropout_rate: 1})
        # Display logs per each loss
        if epoch % display_step == 0:
            print("Epoch:", '%04d' % (epoch + 1), "cost=", "{:.9f}".format(avg_cost))
    print("Optimization Finished!")
    save_path = saver.save(sess, "./model_param.ckpt")

# Peformance checking
    RMSE = tf.sqrt(tf.reduce_mean(tf.square(hypothesis-Y)))
    print("RMSE_train:", sess.run(RMSE, feed_dict={X: train_x, Y: train_y, dropout_rate: 0.5}))
    print("RMSE_test:", sess.run(RMSE, feed_dict={X: test_x, Y: test_y, dropout_rate: 1}))
    answer = hypothesis
    train_pred = sess.run(answer, feed_dict={X: train_x, dropout_rate: 1})
